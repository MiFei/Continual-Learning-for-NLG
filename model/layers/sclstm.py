...
class Sclstm(nn.Module):
	"""
	Implementation of semantic conditioned LSTM
	"""
	...
	def _step(self, input_t, last_hidden, last_cell, last_dt):
		'''
		Feed forward in one step \n
		@param input_t: (batch_size, 1, hidden_size) \n
		@param last_hidden: (batch_size, hidden_size) \n
		@param last_cell: (batch_size, hidden_size) \n
		@return cell, hidden at this time step \n
		'''
	...
	def forward(self, input_seq, last_dt, dataset, gen=False):
		'''
		Feed forward sclstm \n
		@param input_seq: (batch_size, max_len, emb_size) \n
		@param last_dt: (batch_size, feat_size) \n
		@return output_all: (batch_size, max_len, vocab_size) \n
		'''
		...
		input_t = self.get_one_hot_input(batch_size, dataset)
	...
	def get_one_hot_input(self, batch_size, dataset):
